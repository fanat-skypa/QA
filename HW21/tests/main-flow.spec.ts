...
// DOMASHKA
test("500 error without response body", async ({ page }) => {
    await page.route('**/api/loan-calc*', async route => {
...
test("200 status without response body", async ({ page }) => {
    await page.route('**/api/loan-calc*', async route => {
    ...
    const monthlyPayment = page.getByTestId("ib-small-loan-calculator-field-monthlyPayment");
    await expect(monthlyPayment).toContainText("undefined");
});

test.only("200 status with wrong response body key", async ({ page }) => {
    await page.route('**/api/loan-calc*', async route => {
        await route.fulfill({
            status: 200,
            json: { pDiddy: "jeffrey epstein" },
        });
    });
